...
export class CampaignEditComponent implements OnInit {
  ...
  updateCampaign() {
    this.campaignService.updateCampaign(this.campaignDetails.id, this.campaignDetails).subscribe(
      (response) => {
        this._snackBar.open(this.getTranslatedMessage("@@editCampaignSuccessfull"), this.getTranslatedMessage("@@close"), {duration: 3000})
        this.isSuccess = true;
        setTimeout(() => {
          this.router.navigate(['/campaign/list'], {
      ...
      (error) => {
        this._snackBar.open(this.getTranslatedMessage("@@campaignCannotEdit"), this.getTranslatedMessage("@@close"), {duration: 3000})
